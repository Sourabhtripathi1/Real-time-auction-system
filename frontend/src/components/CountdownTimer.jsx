...
const CountdownTimer = ({ endTime }) => {
  const { days, hours, minutes, seconds, isExpired, isEndingSoon, isCritical, isUltraCritical } = useCountdown(endTime);
  ...
  return (
    <div
      className={`relative flex flex-col items-center px-4 py-3 rounded-xl border overflow-hidden ${bgClass} ${isUltraCritical ? "animate-pulse" : ""}`}>
      {isUltraCritical && (
        <div className="absolute inset-0 rounded-xl bg-red-500 opacity-20 animate-ping" />
      )}
