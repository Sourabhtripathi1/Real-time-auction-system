...
const Avatar = ({ user, profileImage, size = 'sm' }) => {
  ...
  return profileImage ? (
    ...
    <div className={`${dim} rounded-full bg-indigo-600 dark:bg-indigo-500 flex items-center justify-center ring-2 ring-indigo-400 dark:ring-indigo-500 text-white font-semibold cursor-pointer flex-shrink-0`}>
...
// ═══════════════════════════════════════════════════════════
const Navbar = () => {
  const { user, profileImage, isAuthenticated, logout } = useAuth();
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-8">
              <Link to="/" className="flex items-center gap-2 group">
                ...
                <span className="text-lg font-bold text-gray-900 dark:text-white tracking-tight">AuctionHub</span>
              </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={toggleTheme} aria-label="Toggle theme"
                ...
                <span className={`absolute transition-all duration-300 ${isDark ? 'opacity-100 rotate-0' : 'opacity-0 -rotate-90'}`}><SunIcon /></span>
                <span className={`absolute transition-all duration-300 ${isDark ? 'opacity-0 rotate-90' : 'opacity-100 rotate-0'}`}><MoonIcon /></span>
              </button>
              ...
              {isAuthenticated ? (
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  {/* Dropdown Panel */}
                  <div className={`absolute right-0 top-12 w-60 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 z-50 overflow-hidden transition-all duration-200 origin-top-right ${
                    isDropdownOpen
                    ...
                    <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800/60 border-b border-gray-100 dark:border-gray-800 flex items-center gap-3">
                      <Avatar user={user} profileImage={profileImage} size="md" />
                      <div className="min-w-0">
                        <p className="font-semibold text-sm text-gray-900 dark:text-white truncate">{user?.name}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{user?.email}</p>
                        <span className={`mt-1 inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full ${roleBadgeColors[user?.role] || ''}`}>
                          {user?.role}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="px-4 py-2.5 border-b border-gray-100 dark:border-gray-800">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400">{completion}%</span>
                      </div>
                    ...
                    <div className="py-1">
                      ...
                      >
                        <span className="text-red-400 dark:text-red-500"><LogoutIcon /></span>
                        Logout
                      </button>
                    </div>
                  </div>
