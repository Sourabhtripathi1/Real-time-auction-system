...
const useProfile = () => {
  ...
  const handleRemoveImage = useCallback(async () => {
    try {
      ...
      setProfile((prev) => ({
        ...prev,
        user: { ...prev?.user, profileImage: null, profileCompletion: updatedUser?.profileCompletion ?? prev?.user?.profileCompletion },
      }));
      updateProfileImage(null);
      return { success: true };
    } catch (err) {
      return { success: false, error: err.response?.data?.message || 'Failed to remove image' };
    }
  }, [updateProfileImage]);
