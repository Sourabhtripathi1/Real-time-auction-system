...
// ── Status config ──────────────────────────────────────────
const STATUS_CONFIG = {
  inactive: { label: "Draft",          style: "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400" },
  pending:  { label: "Pending Review", style: "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400" },
  rejected: { label: "Rejected",       style: "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400" },
  approved: { label: "Approved",       style: "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400" },
  active:   { label: "Live",           style: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" },
  ended:    { label: "Ended",          style: "bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-400" },
};
...
const Spinner = ({ size = "sm" }) => (
  <span className={`inline-block border-2 border-white/30 border-t-white rounded-full animate-spin ${size === "sm" ? "w-3.5 h-3.5" : "w-4 h-4"}`} />
);
    ...
    <tr key={i} className="animate-pulse border-b border-gray-100 dark:border-gray-800">
      {Array.from({ length: cols }).map((__, j) => (
        <td key={j} className="px-4 py-4">
          <div className={`h-4 bg-gray-200 dark:bg-gray-700 rounded ${j === 0 ? "w-8" : j === 1 ? "w-32" : "w-20"}`} />
        </td>
      ))}
    </tr>
  ));
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${cfg.style}`}>
      {status === "active" && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse mr-1.5" />}
      {cfg.label}
    </span>
  );
};
...
const AuctionThumb = ({ auction, onLightbox }) => (
  auction?.images?.length > 0 ? (
    <button onClick={() => onLightbox?.(auction.images)}
      ...
      {auction.images.length > 1 && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
          <span className="text-white text-[10px] font-bold">+{auction.images.length - 1}</span>
        </div>
      )}
    </button>
  ) : (
...
const SellerAvatar = ({ seller }) => {
  ...
  return seller?.profileImage?.url && !err ? (
    <img src={seller.profileImage.url} alt={seller.name} onError={() => setErr(true)}
...
const SortTh = ({ label, field, filters, className = "" }) => {
  ...
  const toggle   = () => filters.setSort(field, isActive && !isAsc ? "asc" : isActive ? "desc" : "desc");
  return (
    <th onClick={toggle}
      ...
      <span className="flex items-center gap-1">
        {label}
        <span className={`transition-opacity ${isActive ? "opacity-100 text-indigo-500" : "opacity-0 group-hover:opacity-50"}`}>
          {isActive ? (isAsc ? " ↑" : " ↓") : " ↕"}
        </span>
      </span>
    </th>
...
const Th = ({ children, className = "" }) => (
  <th className={`px-4 py-3 text-left font-semibold text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 whitespace-nowrap ${className}`}>
    {children}
  </th>
);

// ── Empty state ────────────────────────────────────────────
const EmptyState = ({ hasFilters, onClearFilters, onCreate, emptyTitle, emptySubtitle, emptyAction }) => (
  <tr>
    <td colSpan={99}>
      <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
        {hasFilters ? (
          <>
            ...
            <p className="text-base font-semibold text-gray-700 dark:text-gray-300 mb-1">No results match your filters</p>
          ...
          <>
            ...
            <p className="text-base font-semibold text-gray-700 dark:text-gray-300 mb-1">{emptyTitle || "Nothing here yet"}</p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">{emptySubtitle || ""}</p>
            {onCreate && (
...
const AuctionActions = ({ auction, onEdit, onSubmit, onDelete }) => {
  ...
  const handleSubmit = async () => {
    ...
    try { await onSubmit(auction._id); } finally { setSubmitLoading(false); }
  };
  const handleDelete = async () => {
    setDeleteLoading(true);
    try { await onDelete(auction._id); } finally { setDeleteLoading(false); setConfirmDelete(false); }
  };

  if (isLocked) return (
    <span title={`Cannot modify — auction is ${status}`}
      className="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-600 cursor-not-allowed">
      🔒 Locked
    </span>
  );

  if (confirmDelete) return <DeleteConfirm onConfirm={handleDelete} onCancel={() => setConfirmDelete(false)} loading={deleteLoading} />;

  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      {status === "pending" && (
        <span className="text-xs text-amber-600 dark:text-amber-400 font-medium whitespace-nowrap">⏳ Under Review</span>
      )}
      {status === "rejected" && auction.rejectionReason && (
        <button onClick={() => toast.error(`❌ Rejection: ${auction.rejectionReason}`, { autoClose: 8000 })}
...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    ...
    const onBidUpdated = ({ auctionId, highestBid }) => {
      setAuctions((prev) =>
        prev.map((a) => a._id === auctionId ? { ...a, currentHighestBid: highestBid } : a)
      );
    ...
    const onAuctionEnded = ({ auctionId }) => {
      ...
      setSummary((prev) => ({ ...prev, active: Math.max(0, (prev.active || 1) - 1), ended: (prev.ended || 0) + 1 }));
    };
    socket.on("bidUpdated", onBidUpdated);
    socket.on("auctionEnded", onAuctionEnded);
    return () => { socket.off("bidUpdated", onBidUpdated); socket.off("auctionEnded", onAuctionEnded); };
  }, [socket]);
  ...
  const handleEditSave = async (auctionId, formData, previousStatus) => {
    ...
    setAuctions((prev) => prev.map((a) => a._id === auctionId ? res.data : a));
    if (["pending", "rejected"].includes(previousStatus)) {
      toast.warning("⚠️ Auction moved to Draft. Please resubmit for verification.");
    } else {
  ...
  const handleSubmitForReview = async (auctionId) => {
    try {
      ...
      setAuctions((prev) => prev.map((a) => a._id === auctionId ? { ...a, status: "pending" } : a));
      setSummary((prev) => ({ ...prev, inactive: Math.max(0, (prev.inactive || 1) - 1), pending: (prev.pending || 0) + 1 }));
  ...
  const handleDelete = async (auctionId) => {
    try {
      ...
      setTimeout(() => {
        ...
        setFadingId(null);
        setSummary((prev) => ({ ...prev, total: Math.max(0, (prev.total || 1) - 1) }));
      }, 300);
  ...
  return (
    <>
      {editingAuction && (
        <EditAuctionModal auction={editingAuction} onClose={() => setEditingAuction(null)} onSave={handleEditSave} />
      )}
      {isCreateOpen && (
        <CreateAuctionModal onClose={() => setIsCreateOpen(false)} onSave={handleCreateSave} />
      )}
      {lightboxImages && (
        <LightboxModal images={lightboxImages} onClose={() => setLightboxImages(null)} />
      )}
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">My Auctions</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage and track all your auction listings</p>
          </div>
        ...
        {/* Status tabs */}
        <StatusTabs tabs={sellerTabs} activeTab={filters.filters.status} onChange={filters.setStatus} />
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {isLoading ? (
                  ...
                  const isEndingSoon = a.status === "active" && new Date(a.endTime) - Date.now() < 3_600_000;
                  ...
                  return (
                    <tr key={a._id}
                      ...
                      <td className="px-4 py-3 text-xs text-gray-400 dark:text-gray-600 font-mono">{rowNum}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="font-medium text-gray-900 dark:text-white text-sm leading-tight truncate max-w-[180px]" title={a.title}>
                              {a.title}
                            </p>
                      ...
                      {/* Price */}
                      <td className="px-4 py-3">
                        <p className="text-xs text-gray-400 dark:text-gray-500">Base: ₹{a.basePrice?.toLocaleString("en-IN")}</p>
                        {a.currentHighestBid > 0 ? (
                          <p className={`font-semibold text-sm mt-0.5 transition-colors ${isFlashing ? "text-emerald-600 dark:text-emerald-400" : "text-indigo-600 dark:text-indigo-400"}`}>
                            ₹{a.currentHighestBid?.toLocaleString("en-IN")}
                          </p>
                        ) : (
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">No bids</p>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <p className="text-xs text-gray-500 dark:text-gray-400">{fmtDate(a.startTime)}</p>
                        <p className={`text-xs mt-0.5 ${isEndingSoon ? "text-red-500 dark:text-red-400 font-semibold" : "text-gray-500 dark:text-gray-400"}`}>
                          → {fmtDate(a.endTime)}
                          {isEndingSoon && " ⚡"}
                        </p>
                      </td>
...
const AdminDashboard = () => {
  ...
  useEffect(() => { fetchPending(pendingFilters.buildQueryString()); }, [pendingFilters.filters]); // eslint-disable-line

  // Re-fetch when all filters change
  useEffect(() => { fetchAll(allFilters.buildQueryString()); }, [allFilters.filters]); // eslint-disable-line

  // Auto-refresh pending every 60s
  useEffect(() => {
    const id = setInterval(() => { if (adminTab === "pending") fetchPending(pendingFilters.buildQueryString()); }, 60_000);
    return () => clearInterval(id);
  }, [adminTab, pendingFilters.filters]); // eslint-disable-line

  // Socket: update current bid in All tab
  useEffect(() => {
    if (!socket) return;
    const onBidUpdated = ({ auctionId, highestBid }) => {
      setAllAuctions((prev) => prev.map((a) => a._id === auctionId ? { ...a, currentHighestBid: highestBid } : a));
    };
    socket.on("bidUpdated", onBidUpdated);
  ...
  const handleApprove = async (id) => {
    ...
    } catch { toast.error("❌ Failed to approve auction"); }
  ...
  const handleReject = async (id, reason) => {
    ...
    try {
      ...
      fetchAll(allFilters.buildQueryString());
    } catch { toast.error("❌ Failed to reject auction"); }
    finally { setActionLoading(null); }
  };
  ...
  const AuctionCell = ({ a }) => (
    <div className="flex items-center gap-3 min-w-0">
      <AuctionThumb auction={a} onLightbox={setLightboxImages} />
      <div className="min-w-0">
        <p className="font-medium text-gray-900 dark:text-white text-sm truncate max-w-[160px]" title={a.title}>{a.title}</p>
        <p className="text-xs text-gray-400 font-mono mt-0.5">#{a._id.slice(-6)}</p>
      </div>
    </div>
  );
  ...
  const SellerCell = ({ seller }) => (
    <div className="flex items-center gap-2 min-w-0">
      ...
      <div className="min-w-0">
        <p className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{seller?.name || "—"}</p>
        <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{seller?.email || ""}</p>
      </div>
    </div>
  );
  ...
  const ApproveRejectBtns = ({ a, variant = "normal" }) => {
    ...
    return (
      <div className={`flex gap-1.5 ${busy ? "opacity-60" : ""}`}>
        {variant === "review" && (
          <button onClick={() => setReviewModalAuction(a)} disabled={busy}
        ...
        <button onClick={() => handleApprove(a._id)} disabled={busy}
        ...
        <button onClick={() => setRejectModalAuction(a)} disabled={busy}
  ...
  return (
    <>
      ...
      {lightboxImages && <LightboxModal images={lightboxImages} onClose={() => setLightboxImages(null)} />}
      ...
      <div className="space-y-6">
          ...
          <button onClick={() => setAdminTab("pending")}
            ...
            {pendingCount > 0 && (
              <span className={`px-1.5 py-0.5 text-xs font-bold rounded-full ${
                adminTab === "pending" ? "bg-white/20 text-white" : "bg-amber-100 text-amber-700"
              }`}>{pendingCount}</span>
            )}
          </button>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                {pendingCount} auction{pendingCount !== 1 ? "s" : ""} awaiting review
              </p>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-800">
                    <tr>
                      ...
                      <Th>Auction</Th>
                      <Th>Seller</Th>
                      <SortTh label="Base Price" field="basePrice" filters={pendingFilters} />
                      <Th>Duration</Th>
                      <SortTh label="Submitted" field="createdAt" filters={pendingFilters} />
                      <Th>Actions</Th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {pendingLoad ? (
                      ...
                      const busy   = actionLoading === a._id;
                      const rowNum = (pendingFilters.filters.page - 1) * pendingFilters.filters.limit + idx + 1;
                      return (
                        <tr key={a._id}
                          ...
                          <td className="px-4 py-3"><AuctionCell a={a} /></td>
                          <td className="px-4 py-3"><SellerCell seller={a.seller} /></td>
        ...
        {adminTab === "all" && (
          <div className="space-y-4">
            ...
            <StatusTabs tabs={allTabs} activeTab={allFilters.filters.status} onChange={allFilters.setStatus} />
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-800">
                    <tr>
                      <Th className="w-8">#</Th>
                      <SortTh label="Auction" field="createdAt" filters={allFilters} />
                      <Th>Seller</Th>
                      <SortTh label="Bid / Base" field="currentHighestBid" filters={allFilters} />
                      <Th>Status</Th>
                      <SortTh label="Timeline" field="endTime" filters={allFilters} />
                      <Th>Actions</Th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {allLoad ? (
                      ...
                      const rowNum = (allFilters.filters.page - 1) * allFilters.filters.limit + idx + 1;
                      const busy   = actionLoading === a._id;
                      return (
                        <tr key={a._id}
                          className={`hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors ${busy ? "opacity-60" : ""}`}>
                          ...
                          <td className="px-4 py-3"><SellerCell seller={a.seller} /></td>
                          <td className="px-4 py-3">
                            <p className="text-xs text-gray-400">Base: ₹{a.basePrice?.toLocaleString("en-IN")}</p>
                            {a.currentHighestBid > 0 && (
                          ...
                          <td className="px-4 py-3">
                            {a.status === "pending" && <ApproveRejectBtns a={a} />}
                            {a.status === "active" && (
                              <Link to={`/auction/${a._id}`} target="_blank"
                            ...
                            {a.status === "rejected" && a.rejectionReason && (
                              <span className="text-xs text-red-500 dark:text-red-400 max-w-[140px] truncate block" title={a.rejectionReason}>
                                ⚠ {a.rejectionReason}
                              </span>
                            )}
                          </td>
...
const BidderDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">My Bid History</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Track all your bids across auctions</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          onChange={(e) => { const [f, o] = e.target.value.split("_"); filters.setSort(f, o); }}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {isLoading ? (
                ...
                const a             = bid.auction;
                const rowNum        = (filters.filters.page - 1) * filters.filters.limit + idx + 1;
                const isWinner      = a?.status === "ended" && a?.highestBidder?.toString() === user?._id?.toString();
                ...
                return (
                  <tr key={bid._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    <td className="px-4 py-3 text-xs text-gray-400 font-mono">{rowNum}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3 min-w-0">
                        {a?.images?.length > 0 ? (
                          <img src={a.images[0].url} alt={a.title}
                            className="w-10 h-10 rounded-lg object-cover shrink-0"
                            onError={(e) => { e.target.style.display = "none"; }} />
                        ) : (
                          <div className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 shrink-0" />
                        )}
                        <p className="font-medium text-gray-900 dark:text-white truncate max-w-[150px]" title={a?.title}>
                          {a?.title || "—"}
                        </p>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      {isHighest && (
                        <span className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">✓ Highest</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {isActive ? (
                        <button onClick={() => navigate(`/auction/${a?._id}`)}
